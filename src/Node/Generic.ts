...
export default class Generic {
  ...
  _downloadAssets = async ({
    ...
    const material = await VoxelMaterial.loadAsync(texture);
  ...
  _downloadGLBAsset = async ({ glb, castShadow, receiveShadow }) => {
    const loader = new GLTFLoader();
    const uri = Asset.fromModule(glb).uri;
    const gltf = await loader.loadAsync(uri);
    const model = gltf.scene;
    setShadows(model, { castShadow, receiveShadow });
    return model;
  };

  _download = async (props) => {
    if (props.glb) {
      return await this._downloadGLBAsset(props);
    }
