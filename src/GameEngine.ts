...
export default class Engine {
  ...
  setupGame = (character) => {
    ...
    // Mesh
    this._hero = new PlayerHero(character);
  ...
  // Brings the hero back to life on a safe grass cell at/just behind the
  // current position, clearing the death animation state so play can resume.
  reviveHero = () => {
    const hero = this._hero;
    const z = Math.round(hero.position.z);

    // Find the nearest grass row at or behind the current row.
    let safeZ = z;
    for (let i = 0; i <= 8; i++) {
      const row = this.gameMap.getRow(z - i);
    ...
    // Pick a clear cell on that row (prefer center).
    let x = 0;
    const row = this.gameMap.getRow(safeZ);
    if (row && row.type === "grass") {
      const blocked = new Set(row.entity.getBlockedPositions());
      if (blocked.has(0)) {
        for (let cand = 1; cand <= 4; cand++) {
          if (!blocked.has(cand)) { x = cand; break; }
          if (!blocked.has(-cand)) { x = -cand; break; }
        }
      }
    }
    ...
    hero.position.set(x, groundLevel, safeZ);
    hero.initialPosition = null;
    hero.targetPosition = null;
    hero.lastPosition = null;
    hero.idle();
  };

  // Picks up the coin on the cell the hero just landed on (grass rows only).
  collectCoin = () => {
    const row = this.gameMap.getRow(this._hero.position.z);
    if (!row || row.type !== "grass") return;
    if (row.entity.collectCoinAt(Math.round(this._hero.position.x))) {
      AudioManager.playAsync(AudioManager.sounds.coin);
